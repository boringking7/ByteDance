...
public class Clock extends View {
    ...
    private float UNIT_DEGREE = (float) (6 * Math.PI / 180);// 一个小格的度数，将圆分为60份
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                invalidate();//类似于递归，onDraw和invalidate不断相互调用
            }
        }, 1000);
        //方法二
//        postInvalidateDelayed(1000);
    ...
    private void drawNeedles(final Canvas canvas) {
        ...
        // 画分针
        // todo 2: 画分针
        drawPointer(canvas, 1, nowMinutes);//这样每一分钟内秒针变了分钟不会变啊，可以修改吗？
        // 画时针
