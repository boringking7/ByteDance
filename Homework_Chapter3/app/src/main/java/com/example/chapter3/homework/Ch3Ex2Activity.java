...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        //translate
        ObjectAnimator animator5 = ObjectAnimator.ofFloat(target, "Y", target.getY(), 200, -200,target.getY());
...
//        animator5.start();
        //rotation
        ObjectAnimator animator6 = ObjectAnimator.ofFloat(target,
                "rotation", 0, 360);
        animator6.setRepeatCount(ValueAnimator.INFINITE);
        animator6.setInterpolator(new LinearInterpolator());
        animator6.setRepeatMode(ObjectAnimator.REVERSE);
        animator6.setDuration(Integer.parseInt(durationSelector.getText().toString()));
//        animator6.start();
        // TODO ex2-3: 将上面创建的其他 ObjectAnimator 都添加到 AnimatorSet 中
        animatorSet = new AnimatorSet();
//        animatorSet.playSequentially(animator1,animator2,animator3,animator4,animator5,animator6);//实现playSequentially需要把setRepeatCount(ObjectAnimator.INFINITE)注释掉
